...
contract CentryOnchainAgentAccountTest {
    ...
    function testNativeValueLimitIsEnforced() external {
        ...
        (bool ok,) = address(account).call{value: 0.5 ether}(
            abi.encodeCall(account.execute, (address(target), 0.5 ether, abi.encodeCall(target.receiveNative, ())))
        );
        _assertTrue(ok);
        _assertEq(target.received(), 0.5 ether);

        vm.prank(agent);
        (ok,) = address(account).call{value: 1.5 ether}(
            abi.encodeCall(account.execute, (address(target), 1.5 ether, abi.encodeCall(target.receiveNative, ())))
        );
        _assertFalse(ok);
    }
