...
contract CentryLendingPool is Ownable2Step, ReentrancyGuard, Pausable {
    ...
    mapping(address => Reserve) private _reserves;
    ...
    function setReserveRiskParams(
        ...
        Reserve storage reserve = _getReserve(asset);
    ...
    function getReserveConfig(
        ...
        Reserve memory reserve = _getReserveMemory(asset);
    ...
    function supply(
        ...
        uint256 newSupply = _currentSupply(reserve) + amount;

        if (newSupply > reserve.supplyCap) {
            revert CapExceeded();
        }
    ...
    function withdraw(
        ...
        uint256 userScaledSupply = scaledSupply[
            msg.sender
        ][asset];

        if (scaledAmount > userScaledSupply) {
            scaledAmount = userScaledSupply;
        }
    ...
    function borrow(
        ...
        Reserve storage reserve = _accrue(asset);
        uint256 newBorrows = _currentBorrows(reserve) + amount;

        if (newBorrows > reserve.borrowCap) {
            revert CapExceeded();
        }
    ...
    function currentSupply(
        ...
        Reserve memory reserve = _getReserveMemory(asset);
        uint256 index = _projectLiquidityIndex(
            reserve,
            asset
        );
    ...
    function currentBorrow(
        ...
        Reserve memory reserve = _getReserveMemory(asset);
        uint256 index = _projectBorrowIndex(
            reserve,
            asset
        );
    ...
    function utilization(
        ...
        return _utilizationFromAmounts(
            borrowAmount,
            cash
        );
    }
    ...
    function healthFactor(
        ...
        for (uint256 i = 0; i < reserveList.length; ++i) {
            ...
            Reserve memory reserve = _reserves[asset];
            uint256 collateral = (
                scaledSupply[user][asset] *
    ...
    function _repay(
        ...
        uint256 borrowerScaledDebt = scaledBorrow[
            borrower
        ][asset];

        if (scaledAmount > borrowerScaledDebt) {
            scaledAmount = borrowerScaledDebt;
        }
    ...
    function _utilizationFromAmounts(
        ...
        uint256 denominator = cash + borrowAmount;
