...
interface ICentryVeCENTTransferHook { function onVeCENTTransfer(uint256 tokenId, address from, address to) external; }
...
contract CentryVotingEscrow is ERC721, ReentrancyGuard, Ownable2Step, IVotes {
    ...
    struct VoteCheckpoint { uint64 timestamp; uint192 reserved; uint256 bias; uint256 slope; }
    ...
    error AmountTooLarge(); error InvalidDuration(); error InvalidHook(); error LockExpired();
    error LockNotExpired(); error NoLock(); error RewardsControllerAlreadySet(); error TransferHookAlreadySet();
    error ZeroAmount(); error ZeroToken(); error ZeroTreasury(); error RewardsControllerNotSet();
    error DelegationDisabled(); error FutureLookup();
    ...
    constructor(address token_, address treasury_) ERC721("Centry Vote Escrow", "veCENT") Ownable(msg.sender) {
    ...
    function setRewardsController(address controller) external onlyOwner {
        ...
        rewardsController = controller; emit RewardsControllerSet(controller);
    }
    ...
    function setTransferHook(address hook) external onlyOwner {
        ...
        transferHook = hook; emit TransferHookSet(hook);
    }
    ...
    function increaseAmount(uint256 tokenId, uint256 amount) external nonReentrant {
        ...
        _checkpointVotingPower(tokenId); _checkpointAccount(msg.sender); _checkpointTotal();
    ...
    function withdraw(uint256 tokenId) external nonReentrant {
        ...
        _checkpointVotingPower(tokenId); _checkpointAccount(msg.sender); _checkpointTotal();
        _burn(tokenId);
        delete locks[tokenId]; delete lastVotingPowerCheckpoint[tokenId];
        if (early) {
    ...
    function votingPowerTime(uint256 tokenId) public view returns (uint256) {
        Lock memory lock = locks[tokenId]; uint256 accumulated = cumulativeVotingPowerTime[tokenId];
        ...
        uint256 effectiveEnd = uint256(lock.end); if (checkpoint >= effectiveEnd) return accumulated;
        uint256 to = block.timestamp < effectiveEnd ? block.timestamp : effectiveEnd; if (to <= checkpoint) return accumulated;
        uint256 delta = to - checkpoint; uint256 remainingAtCheckpoint = effectiveEnd - checkpoint; uint256 remainingAtEnd = effectiveEnd - to;
        return accumulated + (uint256(lock.amount) * delta * (remainingAtCheckpoint + remainingAtEnd)) / (2 * MAX_LOCK);
    }

    function checkpointVotingPower(uint256 tokenId) external { ownerOf(tokenId); _checkpointVotingPower(tokenId); }

    function _checkpointVotingPower(uint256 tokenId) internal {
        Lock memory lock = locks[tokenId]; uint256 checkpoint = lastVotingPowerCheckpoint[tokenId];
        ...
        if (checkpoint < effectiveEnd) {
            ...
            if (to > checkpoint) {
                uint256 delta = to - checkpoint; uint256 remainingAtCheckpoint = effectiveEnd - checkpoint; uint256 remainingAtEnd = effectiveEnd - to;
                cumulativeVotingPowerTime[tokenId] += (uint256(lock.amount) * delta * (remainingAtCheckpoint + remainingAtEnd)) / (2 * MAX_LOCK);
            }
        }
        lastVotingPowerCheckpoint[tokenId] = uint64(block.timestamp);
    }
    ...
    function getVotes(address account) public view returns (uint256) { return _currentVotingPower(_voteBias[account], _voteSlope[account], block.timestamp); }
    ...
    function _currentVotingPower(uint256 bias, uint256 slope, uint256 timestamp) internal pure returns (uint256) {
        ...
        if (bias <= scaledSlope) return 0;
        return (bias - scaledSlope) / MAX_LOCK;
    }

    function _checkpointAccount(address account) internal { _writeCheckpoint(_voteCheckpoints[account], _voteBias[account], _voteSlope[account]); }
    function _checkpointTotal() internal { _writeCheckpoint(_totalVoteCheckpoints, _totalVoteBias, _totalVoteSlope); }

    function _writeCheckpoint(VoteCheckpoint[] storage checkpoints, uint256 bias, uint256 slope) internal {
        uint64 timestamp = uint64(block.timestamp); uint256 length = checkpoints.length;
        if (length > 0 && checkpoints[length - 1].timestamp == timestamp) {
            checkpoints[length - 1].bias = bias; checkpoints[length - 1].slope = slope;
        } else {
    ...
    function _lookup(VoteCheckpoint[] storage checkpoints, uint64 timepoint) internal view returns (VoteCheckpoint memory) {
        uint256 length = checkpoints.length; if (length == 0 || checkpoints[0].timestamp > timepoint) return VoteCheckpoint(0, 0, 0, 0);
        uint256 low = 0; uint256 high = length;
        while (low < high) { uint256 mid = (low + high) / 2; if (checkpoints[mid].timestamp <= timepoint) low = mid + 1; else high = mid; }
        return checkpoints[low - 1];
    }

    function _addVotingPosition(address account, uint256 amount, uint256 end) internal { _voteBias[account] += amount * end; _voteSlope[account] += amount; }
    function _removeVotingPosition(address account, uint256 amount, uint256 end) internal { _voteBias[account] -= amount * end; _voteSlope[account] -= amount; }
    function _addTotalVotingPosition(uint256 amount, uint256 end) internal { _totalVoteBias += amount * end; _totalVoteSlope += amount; }
    function _removeTotalVotingPosition(uint256 amount, uint256 end) internal { _totalVoteBias -= amount * end; _totalVoteSlope -= amount; }
    ...
    function _update(address to, uint256 tokenId, address auth) internal override returns (address previousOwner) {
        ...
        if (previousOwner != address(0)) {
            ...
            uint256 index = _ownedTokenIndex[tokenId]; uint256 lastIndex = fromTokens.length - 1;
            if (index != lastIndex) { uint256 movedTokenId = fromTokens[lastIndex]; fromTokens[index] = movedTokenId; _ownedTokenIndex[movedTokenId] = index; }
