...
/// @notice Non-transferable vote-escrow NFT with linearly decaying voting power.
contract CentryVotingEscrow is ERC721, ReentrancyGuard {
    ...
    error ZeroAmount(); error InvalidDuration(); error ExistingLock(); error NoLock(); error LockExpired(); error LockNotExpired(); error NonTransferable(); error AmountTooLarge();
    ...
    constructor(address token_) ERC721("Centry Vote Escrow", "veCENT") { require(token_ != address(0), "ZERO_TOKEN"); token = IERC20(token_); }
    ...
    function createLock(uint256 amount, uint256 duration) external nonReentrant returns (uint256 tokenId) {
        ...
        if (tokenIdOf[msg.sender] != 0) revert ExistingLock();
        uint256 end = ((block.timestamp + duration) / WEEK) * WEEK;
        if (end <= block.timestamp || amount > type(uint128).max) revert InvalidDuration();
        token.safeTransferFrom(msg.sender, address(this), amount);
        tokenId = nextTokenId++;
        tokenIdOf[msg.sender] = tokenId;
        locks[tokenId] = Lock(uint128(amount), uint64(end));
    ...
    function increaseAmount(uint256 amount) external nonReentrant {
        ...
        if (tokenId == 0 || ownerOf(tokenId) != msg.sender) revert NoLock();
    ...
    function withdraw() external nonReentrant {
        ...
        delete locks[tokenId]; delete tokenIdOf[msg.sender]; _burn(tokenId);
    ...
    function votingPower(uint256 tokenId) public view returns (uint256) {
        ...
        return (uint256(lock.amount) * (lock.end - block.timestamp)) / MAX_LOCK;
    }
    function votingPowerOf(address account) external view returns (uint256) { uint256 id = tokenIdOf[account]; return id == 0 ? 0 : votingPower(id); }
    function lockedAmount(address account) external view returns (uint256) { uint256 id = tokenIdOf[account]; return id == 0 ? 0 : locks[id].amount; }
    function lockEnd(address account) external view returns (uint256) { uint256 id = tokenIdOf[account]; return id == 0 ? 0 : locks[id].end; }
