...
export async function createAgentKey({ id, agentId, owner, label, keyHash, scopes }) {
  const c = await db();
  await c.execute({
    sql: "INSERT INTO centry_agent_keys (id, agent_id, owner, label, key_hash, scopes_json, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
    args: [id, agentId, owner, label || "External agent", keyHash, JSON.stringify(scopes || ["read"]), new Date().toISOString()],
  });
}
