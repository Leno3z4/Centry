...
import { ACTIONS, ARC_TESTNET_CHAIN_ID, LENDING_POOL, ORACLE, SUPPORTED_ASSETS, actionCatalog, buildAction, parseUint, resolveAsset } from "./agentExecutionCatalog";
...
export async function assertLiveOperator({ rpcUrl, account, operator }) {
  ...
  const authorized = await contract.agentOperators(normalizedOperator);
