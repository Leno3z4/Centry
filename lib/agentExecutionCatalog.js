...
export function buildAction({ account, action, asset, amount, toAsset, minOut, fee, proposalId, support, recipient }) {
  ...
  switch (action) {
    ...
    case "transfer": {
      ...
      return { scope: ACTIONS.transfer.scope, calls: [call(resolvedAsset, ERC20_INTERFACE.encodeFunctionData("transfer", [getAddress(recipient), parsedAmount]))], description: ACTIONS.transfer.description };
    }
    case "castVote": {
