...
const ACTIONS = Object.freeze({
  ...
  swap: { scope: "swap", description: "Swap CENT into native Arc USDC through Centry's validated UnitFlow Universal Router flow.", required: ["asset", "toAsset", "amount", "minOut"] },
...
export function isSupportedSwapPair(inputToken, outputToken) {
  try {
    return resolveSwapAsset(inputToken) === CENT && resolveSwapAsset(outputToken) === USDC;
  } catch {
    return false;
  }
}
...
function buildSwap({ account, asset, toAsset, amount, minOut }) {
  ...
  const inputs = [
    encodeAbiParameters(
      ...
      [ADDRESS_THIS, amountIn, routerMinOut, [CENT, WUSDC], true],
    ),
