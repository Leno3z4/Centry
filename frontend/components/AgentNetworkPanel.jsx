    ...
    <section className={styles.networkPanel}>
      <div className={styles.networkPanelHead}>
        <div>
          ...
          <p>Genesis network · messages only.</p>
        </div>
        <button type="button" className={styles.secondaryButton} onClick={refresh} disabled={state === 'loading'}>
      ...
      {peers.length ? (
        <>
          <div className={styles.networkPeers}>
            {peers.map((peer) => (
              <button
                key={peer.id}
                type="button"
              ...
              >
                <span><strong>{peer.name}</strong><small>{shortAddress(peer.account)}</small></span>
                <span>Message</span>
              </button>
            ))}
          </div>
          <div className={styles.networkComposer}>
            <select className={styles.input} value={selected} onChange={(event) => setSelected(event.target.value)}>
              {peers.map((peer) => <option key={peer.id} value={peer.id}>{peer.name} · {shortAddress(peer.account)}</option>)}
            </select>
            <input
            ...
            />
            <button type="button" className={styles.primaryButton} onClick={send} disabled={!message.trim() || !target}>
              Send
            </button>
          </div>
        </>
      ) : (
        <p className={styles.networkEmpty}>Create another agent to start comparing and coordinating agents.</p>
      )}
      {status ? <p className={styles.networkStatus}>{status}</p> : null}
      {state === 'error' ? <p className={styles.networkStatus}>Network unavailable.</p> : null}
    </section>
  );
}
