...
export default function CentryIntelligence({ market, lending, compact: compactMode = false }) {
  ...
  const ask = async (value) => {
    ...
    setQuestion(nextQuestion); setAnswer(''); setPlan(null); setLoading(true);
    try {
      const response = await fetch('/api/assistant', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ question: nextQuestion, context }) });
    ...
    } catch (error) { setAnswer(error?.message || 'Unable to analyze the request right now.'); }
