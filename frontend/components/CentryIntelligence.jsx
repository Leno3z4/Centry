...
const SUGGESTED = [
  'What can I do with my position?',
  'Show my borrow capacity',
  'What is my market liquidity?',
];
...
export default function CentryIntelligence({ market, lending, compact: compactMode = false }) {
  ...
  const ask = async (value) => {
    ...
    try {
      ...
      if (!response.ok || !result.success) {
        throw new Error(result.error || 'Unable to analyze the request.');
      }

      setAnswer(result.answer || '');
      setPlan(result.plan || null);

      // Position state is intentionally refreshed only after a submitted message.
      // Typing in the composer never triggers an RPC refresh.
  ...
  return (
    <section className={`${styles.card} ${compactMode ? styles.compactCard : ''}`} aria-label="Centrion assistant">
      {!compactMode ? (
        <div className={styles.header}>
          <div>
            ...
            <p>Ask about your position or tell Centrion what you want done inside Centry.</p>
          </div>
          {liquidation.atRisk ? (
            <div className={`${styles.status} ${styles.status_danger}`}>
              <span>Liquidation risk</span>
              <strong>HF {lending.healthFactor}</strong>
            </div>
          ) : null}
        </div>
      ) : null}
      ...
      {compactMode && liquidation.atRisk ? (
      ...
      <div className={styles.chips}>
        {SUGGESTED.map((item) => (
          <button key={item} type="button" onClick={() => ask(item)} disabled={loading}>
            {item}
          </button>
        ))}
      </div>
      ...
      >
        <textarea
          ...
          placeholder="Ask Centrion or tell it what to do…"
