...
import styles from '../app/app/gateway/gateway.module.css';

const ERC20_BALANCE_ABI = [{ type: 'function', name: 'balanceOf', stateMutability: 'view', inputs: [{ name: 'owner', type: 'address' }], outputs: [{ name: '', type: 'uint256' }] }];
...
const addressToBytes32 = (address) => pad(address.toLowerCase(), { size: 32 });
...
function buildSpec(source, destination, address, value) {
  return {
    version: 1,
...
export function GatewayPage() {
  ...
  const [transferSourceId, setTransferSourceId] = useState('arc-testnet');
  const [transferDestinationId, setTransferDestinationId] = useState('base-sepolia');
  const [transferAmount, setTransferAmount] = useState('');
  const [transferFee, setTransferFee] = useState(null);
  const [transferMaxBlockHeight, setTransferMaxBlockHeight] = useState(null);
  ...
  const transferSource = useMemo(() => GATEWAY_TESTNET_CHAINS.find((chain) => chain.id === transferSourceId) || GATEWAY_TESTNET_CHAINS[0], [transferSourceId]);
  const transferDestination = useMemo(() => GATEWAY_TESTNET_CHAINS.find((chain) => chain.id === transferDestinationId) || GATEWAY_TESTNET_CHAINS[1], [transferDestinationId]);
  ...
  const transferAvailableBalance = gatewayBalances.find((chain) => chain.id === transferSource.id)?.balance || '0';
  const transferAmountRaw = validTransferAmount ? parseUnits(transferAmount.trim(), 6) : 0n;
  let sourceAvailableRaw = 0n;
  try { sourceAvailableRaw = parseUnits(String(transferAvailableBalance), 6); } catch { sourceAvailableRaw = 0n; }
  const exceedsGatewayBalance = validTransferAmount && transferAmountRaw > sourceAvailableRaw;
  ...
  const deposit = async () => {
    if (!isConnected || !address || !validAmount || ['approval', 'deposit', 'submitted', 'switching'].includes(stage)) return;
  ...
  const transfer = async () => {
    if (!isConnected || !address || !validTransferAmount || sameTransferChain || exceedsGatewayBalance || transferStage !== 'idle') return;
    setTransferError(''); setTransferNotice(''); setTransferTxHash(''); setTransferFee(null); setTransferMaxBlockHeight(null);
    try {
      ...
      if (!response.ok || !balanceResult.success) throw new Error(balanceResult.error || 'Unable to verify the Gateway balance.');
      const latest = balanceResult.balances?.find((chain) => chain.id === transferSource.id)?.balance || '0';
      if (transferAmountRaw > parseUnits(String(latest), 6)) throw new Error(`Insufficient available Gateway balance on ${transferSource.name}. Available: ${latest} USDC.`);

      await switchToChain(transferSource);
      setTransferStage('estimating'); setTransferNotice(`Estimating Gateway fee for ${transferSource.name} → ${transferDestination.name}.`);
      const spec = buildSpec(transferSource, transferDestination, address, transferAmountRaw);
      const estimateResponse = await fetch('/api/circle/gateway/estimate', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ spec }), cache: 'no-store' });
      const estimateResult = await estimateResponse.json();
      if (!estimateResponse.ok || !estimateResult.success) throw new Error(estimateResult.error || 'Unable to estimate the Gateway transfer.');

      const burnIntent = { maxBlockHeight: String(estimateResult.maxBlockHeight), maxFee: String(estimateResult.maxFee), spec };
      setTransferFee(estimateResult.fees?.total || null); setTransferMaxBlockHeight(burnIntent.maxBlockHeight);
      setTransferStage('signing'); setTransferNotice(`Sign the Gateway burn intent for ${transferSource.name} → ${transferDestination.name}.`);
      const typedData = { types: GATEWAY_EIP712_TYPES, domain: GATEWAY_EIP712_DOMAIN, primaryType: 'BurnIntent', message: burnIntent };
      ...
      const transferResponse = await fetch('/api/circle/gateway/transfer', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ burnIntent, signature }), cache: 'no-store' });
      const transferResult = await transferResponse.json();
      if (!transferResponse.ok || !transferResult.success) throw new Error(transferResult.error || 'Gateway did not issue an attestation.');
      ...
      const mintData = encodeFunctionData({ abi: GATEWAY_MINTER_ABI, functionName: 'gatewayMint', args: [transferResult.attestation, transferResult.signature] });
    ...
    } catch (caughtError) { setTransferError(errorText(caughtError)); setTransferStage('idle'); }
  };
    ...
    <div className={styles.page}>
      ...
      <div className={styles.grid}>
        <section className={styles.card}>
          ...
          <select value={sourceId} onChange={(event) => { setSourceId(event.target.value); setError(''); setNotice(''); setStage('idle'); }}>{GATEWAY_TESTNET_CHAINS.map((chain) => <option key={chain.id} value={chain.id}>{chain.name}</option>)}</select>
        ...
        <section className={styles.card}>
          ...
          <div className={styles.infoBox}><strong>What happens next?</strong><span>Finalized deposits become unified Gateway balance, which can then be transferred to another supported chain through Gateway.</span></div>
        </section>
      </div>
      ...
      <section className={styles.card}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0, 1fr))', gap: 12 }}>
          <div><label className={styles.fieldLabel}>Source balance</label><select value={transferSourceId} onChange={(event) => { setTransferSourceId(event.target.value); setTransferStage('idle'); setTransferError(''); setTransferNotice(''); }}>{GATEWAY_TESTNET_CHAINS.map((chain) => <option key={chain.id} value={chain.id}>{chain.name}</option>)}</select><div className={styles.metaRow}><span>Available</span><strong>{Number(transferAvailableBalance).toLocaleString(undefined, { maximumFractionDigits: 6 })} USDC</strong></div></div>
          <div><label className={styles.fieldLabel}>Destination chain</label><select value={transferDestinationId} onChange={(event) => { setTransferDestinationId(event.target.value); setTransferStage('idle'); setTransferError(''); setTransferNotice(''); }}>{GATEWAY_TESTNET_CHAINS.map((chain) => <option key={chain.id} value={chain.id}>{chain.name}</option>)}</select></div>
          <div><label className={styles.fieldLabel}>Amount</label><div className={styles.amountField}><input type="text" inputMode="decimal" placeholder="0.00" value={transferAmount} onChange={(event) => { const value = event.target.value; if (value === '' || /^\d*(\.\d{0,6})?$/.test(value)) setTransferAmount(value); setTransferError(''); setTransferNotice(''); setTransferStage('idle'); }} /><span>USDC</span></div></div>
          <div><label className={styles.fieldLabel}>Estimated fee</label><div style={{ minHeight: 58, display: 'flex', alignItems: 'center', padding: '0 14px', border: '1px solid #282033', borderRadius: 12, background: '#0b0712', color: '#c8b5e6', font: '11px DM Mono, monospace' }}>{transferFee ? `${Number(transferFee).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 6 })} USDC` : '—'}</div></div>
        </div>
        {sameTransferChain ? <div className={styles.notice}>Choose different source and destination chains for this cross-chain transfer.</div> : null}
        {exceedsGatewayBalance ? <div className={`${styles.notice} ${styles.noticeError}`}>The requested amount exceeds the available finalized Gateway balance on {transferSource.name}.</div> : null}
        ...
        <button type="button" className={styles.primaryButton} disabled={!isConnected || !validTransferAmount || sameTransferChain || exceedsGatewayBalance || transferStage !== 'idle'} onClick={transfer}>{transferButtonLabel}</button>
        <div className={styles.metaRow} style={{ marginTop: 12 }}><span>{transferMaxBlockHeight ? `Intent expiry block: ${transferMaxBlockHeight}` : 'Fee and expiry are estimated immediately before signing.'}</span><span>Source balance is debited when the attestation is used.</span></div>
        <p className={styles.helper}>The wallet signs the exact fee-estimated EIP-712 burn intent. Circle returns the attestation and operator signature; the destination GatewayMinter then mints USDC to the same wallet. Existing CCTP and swap transaction paths are not used.</p>
