...
import { SELF_REPAYING_FACTORY_ABI, SELF_REPAYING_POSITION_ABI } from '../constants/selfRepaying';
import { ERC20_ABI } from '../constants/abis';

export const COLLATERAL_ASSETS = [
  { id: 'ETH', symbol: 'mETH', name: 'Mock Ethereum', address: CONTRACT_ADDRESSES.collateralAssets.ETH, decimals: 18 },
  { id: 'BTC', symbol: 'mBTC', name: 'Mock Bitcoin', address: CONTRACT_ADDRESSES.collateralAssets.BTC, decimals: 18 },
  { id: 'SOL', symbol: 'mSOL', name: 'Mock Solana', address: CONTRACT_ADDRESSES.collateralAssets.SOL, decimals: 18 },
  { id: 'EUR', symbol: 'mEUR', name: 'Mock Euro', address: CONTRACT_ADDRESSES.collateralAssets.EUR, decimals: 18 },
];

export function getCollateralAsset(address) {
  return COLLATERAL_ASSETS.find((asset) => asset.address?.toLowerCase() === address?.toLowerCase()) || null;
}
...
export function useSelfRepayingVault() {
  ...
  const configured = hasAddress('selfRepayingFactory') && hasAddress('yieldVault') && hasAddress('USDC') && COLLATERAL_ASSETS.every((asset) => /^0x[a-fA-F0-9]{40}$/.test(asset.address));
  ...
  const harvestAndRepay = () => sendAndWait({ address: positionAddress, abi: SELF_REPAYING_POSITION_ABI, functionName: 'harvestAndRepay' });
  const closePosition = () => sendAndWait({ address: positionAddress, abi: SELF_REPAYING_POSITION_ABI, functionName: 'closePosition' });

  const refetchAll = async () => {
    const requests = [refetchPositions()];
    if (positionAddress) requests.push(refetchOpen(), refetchCollateral(), refetchYieldPrincipal(), refetchTotalRepaid(), refetchDebt(), refetchYieldAssets(), refetchProfit(), refetchHealth(), refetchAllowance());
