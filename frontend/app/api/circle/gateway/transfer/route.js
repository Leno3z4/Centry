...
export async function POST(request) {
  try {
    ...
    if (!requests.length) return Response.json({ success: false, error: 'Signed Gateway transfer intents are required.' }, { status: 400 });
    ...
    if (!json?.attestation || !json?.signature) return Response.json({ success: false, error: 'Circle Gateway did not return a usable attestation.' }, { status: 502 });
