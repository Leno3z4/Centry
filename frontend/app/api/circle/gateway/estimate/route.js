...
export async function POST(request) {
  try {
    ...
    const response = await fetch(`${CIRCLE_GATEWAY_TESTNET_API}/v1/estimate`, {
      ...
      body: JSON.stringify(specs.length === 1 ? [{ spec: specs[0] }] : [{ specs }]),
      cache: 'no-store',
    });
    ...
    const estimates = Array.isArray(json?.body)
      ? json.body.map((item) => item?.burnIntent || item).filter((item) => item?.maxFee && item?.maxBlockHeight)
      : [];
    const fallback = json?.burnIntent?.maxFee && json?.burnIntent?.maxBlockHeight ? [json.burnIntent] : estimates;

    if (fallback.length !== specs.length) return Response.json({ success: false, error: 'Circle Gateway did not return a usable estimate for every transfer intent.' }, { status: 502 });
    return Response.json({ success: true, estimates: fallback }, { status: 200 });
  } catch (error) {
