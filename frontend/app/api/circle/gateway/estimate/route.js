...
export async function POST(request) {
  ...
  try {
    ...
    const response = await fetch(`${CIRCLE_GATEWAY_API}/v1/estimate`, { method: 'POST', headers: CIRCLE_GATEWAY_HEADERS, body: JSON.stringify(specs.map((spec) => ({ spec }))), cache: 'no-store' });
