...
export async function POST(request) {
  try {
    ...
    const estimated = json?.body?.[0]?.burnIntent;
    if (!estimated?.maxFee || !estimated?.maxBlockHeight) return Response.json({ success: false, error: 'Circle Gateway did not return a usable fee estimate.' }, { status: 502 });

    return Response.json({ success: true, maxFee: estimated.maxFee, maxBlockHeight: estimated.maxBlockHeight, fees: json.fees || null }, { status: 200 });
  } catch (error) {
