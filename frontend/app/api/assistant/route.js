...
import { buildDeterministicExecutionPlan } from '../../../lib/deterministicExecution';
...
function responsePayload({ answer, plan, provider, warning }) { return { success: true, answer, plan: plan || null, provider, ...(warning ? { warning } : {}) }; }
...
export async function POST(request) {
  ...
  try {
    ...
    const deterministicPlan = buildDeterministicExecutionPlan(question);
    const apiKey = process.env.GEMINI_API_KEY;

    if (!apiKey) {
      return withRateLimitHeaders(NextResponse.json(responsePayload({
        answer: deterministicPlan ? `Prepared ${deterministicPlan.title}. Review it below and sign the transaction in your wallet.` : fallbackPositionAnswer(context, question),
    ...
    return withRateLimitHeaders(NextResponse.json(responsePayload({
      answer: parsed.answer || (deterministicPlan ? `Prepared ${deterministicPlan.title}. Review it below and sign the transaction in your wallet.` : fallbackPositionAnswer(context, question)),
      plan: parsed.plan || deterministicPlan,
      provider: model,
    }), limit), limit);
  } catch (error) {
