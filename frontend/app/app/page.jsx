    ...
    return (
        <div className="page-stack">
            <section className="hero">
                <div className="hero-copy">
                    ...
                    <div className="hero-actions">
                        <button type="button" className="primary-btn" onClick={() => setActiveView('lending')}>
                            Open lending
                        </button>
                        <button type="button" className="secondary-btn" onClick={() => setActiveView('portfolio')}>
                            View portfolio
                        </button>
                    </div>
                </div>
            ...
            <section className="stats-grid">
                ...
                <StatCard label="Health factor" value={connected ? formatHealth(lending.healthFactor) : '—'} detail={connected ? 'Your account' : 'Connect wallet'} />
            </section>
...
function Lending({ lending, connected }) {
    ...
    const setMax = () => {
        if (action === 'withdraw') return setAmount(lending.supplyBalance || '0');
        if (action === 'repay') return setAmount(lending.borrowBalance || '0');
    ...
    return (
        <div className="page-stack">
            ...
            <section className="stats-grid">
                ...
                <StatCard label="Health factor" value={connected ? formatHealth(lending.healthFactor) : '—'} detail={connected ? 'Onchain account health' : 'Connect wallet'} />
            </section>
            <section className="workspace-grid">
                <div className="panel workspace-panel">
                    <div className="action-tabs">
                        {['supply', 'withdraw', 'borrow', 'repay'].map((item) => (
                            <button key={item} type="button" className={action === item ? 'active' : ''} onClick={() => setAction(item)}>{item[0].toUpperCase() + item.slice(1)}</button>
                        ))}
                    </div>
                    <div className="form-card">
                        ...
                        <div className="amount-input-wrap">
                            <input id="market-amount" type="number" min="0" step="0.000001" placeholder="0.00" value={amount} onChange={(event) => setAmount(event.target.value)} />
                            <span>mUSDC</span>
                        </div>
                        <div className="form-meta">
                            <span>Wallet: {connected ? `${formatNumber(lending.usdcBalance)} mUSDC` : 'Connect wallet'}</span>
                        ...
                        {!connected ? (
                            ...
                            <button type="button" className="primary-btn full-btn large-btn" disabled={busy || !amount} onClick={run}>
                                {busy ? 'Waiting for confirmation…' : needsApproval ? `Approve mUSDC for ${action}` : `${action[0].toUpperCase()}${action.slice(1)} mUSDC`}
                            </button>
                        )}
                ...
                <div className="panel position-panel">
                    ...
                    <div className="position-list">
                        ...
                        <div className="position-row"><span>Health factor</span><strong>{connected ? formatHealth(lending.healthFactor) : '—'}</strong></div>
                    </div>
                </div>
            </section>
        </div>
        ...
        <div className="page-stack">
            ...
            <div className="panel">
                ...
                <div className="risk-card">
                    <div className="risk-value"><span>Health factor</span><strong>{connected ? formatHealth(lending.healthFactor) : '—'}</strong></div>
                    <p>Health factor is read from the deployed Centry lending pool for the connected wallet.</p>
                </div>
            </div>
        </div>
    );
...
function Rewards() {
    return (
        <div className="page-stack">
            <div className="section-header"><div><span className="section-kicker">REWARDS</span><h1>Protocol rewards</h1><p>Claims stay inactive until an eligible revenue epoch and proof set exist.</p></div></div>
            <div className="feature-grid">
                <div className="panel feature-card"><span className="section-kicker">REVENUE</span><h2>Revenue distributor</h2><p>The deployed distributor supports proof-based claims once an eligible epoch is configured.</p></div>
                <div className="panel feature-card"><span className="section-kicker">INCENTIVES</span><h2>Liquidity incentives</h2><p>Future market incentives can be added without changing the lending-pool core.</p></div>
                <div className="panel feature-card"><span className="section-kicker">GOVERNANCE</span><h2>veCENT utility</h2><p>veCENT provides the governance position used by future reward-routing mechanics.</p></div>
            </div>
        </div>
    );
}
...
function Analytics({ lending }) {
    return (
        <div className="page-stack">
            ...
            <div className="panel empty-chart-panel"><span className="section-kicker">HISTORY</span><h2>Historical charts</h2><p>Historical graphs require indexed onchain events. Centry will not fabricate a historical series.</p></div>
        </div>
    );
}
...
function Docs() {
    return (
        <div className="page-stack">
            <div className="section-header"><div><span className="section-kicker">DOCS</span><h1>Centry documentation</h1><p>Current testnet architecture and user flows.</p></div></div>
            <div className="docs-grid">
                {[
                    ...
                    <div className="panel doc-card" key={title}>
                        <span className="section-kicker">{title.toUpperCase()}</span><h2>{title}</h2>
                        <div className="doc-list">{items.map((item) => <div className="doc-item" key={item}><span className="doc-marker">•</span><strong>{item}</strong></div>)}</div>
                    </div>
                ))}
            </div>
        </div>
...
function Dashboard() {
    ...
    return (
        <AppShell activeView={activeView} setActiveView={setActiveView} chainId={chainId} address={address}>
            {content}
        </AppShell>
    );
}
