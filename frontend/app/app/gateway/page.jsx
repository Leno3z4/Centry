...
function errorText(error) { return error?.shortMessage || error?.message || 'The Gateway transaction could not be completed.'; }
...
function chainLabel(chainId) { return GATEWAY_TESTNET_CHAINS.find((chain) => chain.chainId === chainId)?.name || `Chain ${chainId}`; }
function toBytes32(address) { return pad(address.toLowerCase(), { size: 32 }); }
...
function GatewayContent() {
  ...
  const refreshGatewayBalances = useCallback(async () => {
    if (!address) { setGatewayBalances([]); setGatewayTotal('0.000000'); return; }
    ...
    try {
      ...
      setGatewayBalances(Array.isArray(result.balances) ? result.balances : []); setGatewayTotal(result.total || '0.000000');
    } catch (caughtError) { setError(errorText(caughtError)); } finally { setLoadingBalances(false); }
  }, [address]);
  ...
  const refreshWalletBalance = useCallback(async () => {
    ...
    } catch (caughtError) { setWalletBalance(null); setError(errorText(caughtError)); }
  ...
  const transfer = async () => {
    ...
    try {
      ...
      const burnIntent = {
        maxBlockHeight: MAX_UINT256.toString(), maxFee: MAX_GATEWAY_FEE.toString(),
        spec: {
          version: 1, sourceDomain: transferSource.domain, destinationDomain: transferDestination.domain,
          sourceContract: toBytes32(GATEWAY_WALLET_ADDRESS), destinationContract: toBytes32(GATEWAY_MINTER_ADDRESS),
          sourceToken: toBytes32(transferSource.usdc), destinationToken: toBytes32(transferDestination.usdc),
          sourceDepositor: toBytes32(address), destinationRecipient: toBytes32(address), sourceSigner: toBytes32(address),
          destinationCaller: toBytes32(ZERO_ADDRESS), value: value.toString(), salt: randomBytes32(), hookData: '0x',
        },
      };
      setTransferStage('signing'); setTransferNotice(`Sign the Gateway burn intent for ${transferSource.name} → ${transferDestination.name}.`);
      const typedData = { domain: GATEWAY_EIP712_DOMAIN, types: GATEWAY_EIP712_TYPES, primaryType: 'BurnIntent', message: burnIntent };
      ...
      const response = await fetch('/api/circle/gateway/transfer', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ burnIntent, signature }), cache: 'no-store' });
      const result = await response.json();
      ...
      const mintData = encodeFunctionData({ abi: GATEWAY_MINTER_ABI, functionName: 'gatewayMint', args: [result.attestation, result.signature] });
  ...
  return (
    <div className={styles.page}>
      <header className={styles.header}><div><span className={styles.kicker}>CENTRY · CIRCLE GATEWAY</span><h1>Unified USDC</h1><p>Deposit USDC into Circle Gateway and make finalized liquidity available as one cross-chain balance.</p></div><span className={styles.statusPill}><i /> Arc Testnet</span></header>
      <div className={styles.grid}>
        <section className={styles.card}>
          ...
          <label className={styles.fieldLabel}>Source chain</label><select value={sourceId} onChange={(event) => { setSourceId(event.target.value); setError(''); setNotice(''); setStage('idle'); }}>{GATEWAY_TESTNET_CHAINS.map((chain) => <option key={chain.id} value={chain.id}>{chain.name}</option>)}</select>
          <label className={styles.fieldLabel}>Amount</label><div className={styles.amountField}><input type="text" inputMode="decimal" placeholder="0.00" value={amount} onChange={(event) => { const value = event.target.value; if (value === '' || /^\d*(\.\d{0,6})?$/.test(value)) setAmount(value); }} /><span>USDC</span></div>
          ...
          {notice ? <div className={`${styles.notice} ${stage === 'submitted' ? styles.noticeSuccess : ''}`}>{notice}</div> : null}{error ? <div className={`${styles.notice} ${styles.noticeError}`} role="alert">{error}</div> : null}
      ...
      <section className={styles.card}>
        ...
        <div className={styles.transferGrid}>
          <div><label className={styles.fieldLabel}>Source balance</label><select value={transferSourceId} onChange={(event) => { setTransferSourceId(event.target.value); setTransferError(''); setTransferNotice(''); }}>{GATEWAY_TESTNET_CHAINS.map((chain) => <option key={chain.id} value={chain.id}>{chain.name}</option>)}</select></div>
          <div><label className={styles.fieldLabel}>Destination chain</label><select value={transferDestinationId} onChange={(event) => { setTransferDestinationId(event.target.value); setTransferError(''); setTransferNotice(''); }}>{GATEWAY_TESTNET_CHAINS.map((chain) => <option key={chain.id} value={chain.id}>{chain.name}</option>)}</select></div>
          <div><label className={styles.fieldLabel}>Amount</label><div className={styles.amountField}><input type="text" inputMode="decimal" placeholder="0.00" value={transferAmount} onChange={(event) => { const value = event.target.value; if (value === '' || /^\d*(\.\d{0,6})?$/.test(value)) setTransferAmount(value); }} /><span>USDC</span></div></div>
          <div className={styles.transferAction}><button type="button" className={styles.primaryButton} disabled={!isConnected || !validTransferAmount || sameTransferChain || transferStage !== 'idle'} onClick={transfer}>{transferButtonLabel}</button></div>
        </div>
        ...
        {transferNotice ? <div className={`${styles.notice} ${transferStage === 'confirmed' ? styles.noticeSuccess : ''}`}>{transferNotice}</div> : null}{transferError ? <div className={`${styles.notice} ${styles.noticeError}`} role="alert">{transferError}</div> : null}
