...
async function requestJson(url, options, { retries = 0, retryStatuses = [] } = {}) {
  let lastError;
    ...
    <div className={`${styles.chainPicker} ${open ? styles.chainPickerOpen : ''}`} ref={rootRef}>
      ...
      {open && (
        <div className={styles.chainMenu} role="listbox" aria-label={label}>
          {chains.map((chain) => (
            <button key={chain.id} type="button" role="option" aria-selected={chain.id === selected.id} className={`${styles.chainOption} ${chain.id === selected.id ? styles.chainOptionActive : ''}`} onClick={() => { onChange(chain.id); setOpen(false); }}>
...
function BridgeContent() {
  ...
  const detectBalance = async () => {
    ...
    setError('');
    try {
      const { response, data } = await requestJson('/api/tower/wallet/balance', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ address, chainId: source.id }),
      }, { retries: 2, retryStatuses: [429, 500, 502, 503, 504] });
      if (!response.ok || data?.success === false) throw new Error(data?.error || 'Could not read the source-chain balance.');
  ...
  const bridge = async () => {
    ...
    try {
      ...
      }, { retries: 1, retryStatuses: [] });

      if (!response.ok || data?.success === false) throw new Error(data?.error || 'Tower could not create the bridge transfer.');
      setResult(data);
    } catch (caughtError) {
  ...
  return (
    <div className={styles.page}>
      <header className={styles.header}>
        <div><span className={styles.kicker}>CENTRY · BRIDGE</span><h1>Move USDC across chains</h1><p>Bridge testnet USDC into or out of Arc using Circle CCTP via Tower.</p></div>
      ...
      <section className={styles.card}>
        ...
        <button type="button" className={styles.arrowButton} onClick={switchDirection} disabled={loading} aria-label="Switch bridge direction" title="Switch bridge direction">⇅</button>
        ...
        {error && <div className={`${styles.notice} ${styles.noticeError}`} role="alert">{error}{!loading && validAmount && isConnected ? <button type="button" className={styles.refreshButton} onClick={bridge}>Retry bridge</button> : null}</div>}
        {result && <div className={`${styles.notice} ${styles.noticeSuccess}`}><strong>Bridge requested.</strong><span>{result.status ? `Status: ${result.status}` : 'Tower accepted the request.'}</span>{result.transactionHash && <code>{result.transactionHash}</code>}{result.estimatedTime && <span>Estimated settlement: {result.estimatedTime}</span>}</div>}
      </section>
