...
export default function CreateAgentPage() {
  ...
  if (createdAccount) {
    return <Providers><AppShell><main className={styles.page}>{status ? <div className={styles.notice}>{status}</div> : null}{error ? <div className={styles.error}>{error}</div> : null}<section className={styles.successHero}><div className={styles.successMark}>✓</div><p className={styles.kicker}>Agent ready</p><h1>{setupComplete ? 'Your agent is created' : 'Your smart account is created'}</h1><p>{setupComplete ? 'The smart account is owned by your connected wallet. It starts OFF until you activate it.' : 'The blockchain transaction succeeded, but the off-chain setup still needs attention. You can continue from the agent configuration page.'}</p><div className={styles.addressPanel}><span>Smart account</span><code>{createdAccount}</code></div><div className={styles.headerActions}><button className={styles.secondaryButton} type="button" onClick={() => navigator.clipboard.writeText(createdAccount)}>Copy address</button><a className={styles.secondaryButton} href={`https://explorer.arc.io/address/${createdAccount}`} target="_blank" rel="noreferrer">View on Arc</a><button className={styles.primaryButton} type="button" onClick={() => router.push(`/app/agents/${createdAccount}`)}>Open agent</button><button className={styles.secondaryButton} type="button" onClick={() => router.push(`/app/agents/${createdAccount}/configure`)}>Configure</button></div></section></main></AppShell></Providers>;
  }
