...
async function main() {
  if (!RPC_URL) {
    fail("Missing ARC_RPC_URL");
  }

  if (!PRIVATE_KEY) {
    fail("Missing CENTRY_REVENUE_REWARDS_OWNER_PRIVATE_KEY");
  }

  if (!fs.existsSync(MANIFEST_PATH)) {
    fail(`${MANIFEST_PATH} does not exist`);
  }
  ...
  if (epoch <= 0n) {
    fail("Manifest epoch must be greater than zero");
  }

  if (!ethers.isHexString(manifestRoot, 32)) {
    fail("Manifest root must be a 32-byte hex value");
  }

  if (rewardBudget <= 0n) {
    fail("Manifest reward budget must be greater than zero");
  }
  ...
  if (pending.root === ethers.ZeroHash) {
    fail(`Epoch ${epoch.toString()} is not queued`);
  }

  if (pending.root.toLowerCase() !== manifestRoot.toLowerCase()) {
    fail("Pending epoch root does not match manifest root");
  }

  if (BigInt(pending.rewardBudget) !== rewardBudget) {
    fail("Pending epoch reward budget does not match manifest budget");
  }
  ...
  if (activatedLatest < epoch) {
    fail("Activation transaction confirmed but latestEpoch did not advance");
  }

  if (activatedRoot.toLowerCase() !== manifestRoot.toLowerCase()) {
    fail("Activation confirmed but on-chain root does not match manifest");
  }

  if (activatedBudget !== rewardBudget) {
    fail("Activation confirmed but on-chain reward budget does not match manifest");
  }
